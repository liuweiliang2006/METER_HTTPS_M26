...
OpenLockPacket_t openLockPacket;
bool IsNeedReportOpenLock = false;
...
//给控制需要回复的包赋值
void refreshOpenLockPacket(OpenLockPacket_t *rPacket)
{
	  //sumCnt = 0;
	  //long date,time;//UTC
	  //GetRTC(&time,&date);
...
//	stmerncpy(waringPackage->meterNumer,Sim80x.IMEI,15);
    ...
    sprintf(rPacket->identifier,"%04d",identCnt++);
		strncpy(rPacket->CARD_ID,REAL_DATA_PARAM.CURR_BlueCard_ID,18);
    strncpy(rPacket->datetime,REAL_DATA_PARAM.OpenLockDatatime,12);
}
//编码控制回复包
void encodeOpenLockPacket(char *sendMeagess,OpenLockPacket_t *rPacket)
{
	  strncat(sendMeagess,"##",2);
	  strncat(sendMeagess,"MELO",4);
    strncat(sendMeagess,"/",1);
    strncat(sendMeagess,rPacket->meterNumer,10);
    strncat(sendMeagess,"/",1);
    strncat(sendMeagess,rPacket->identifier,4);
    strncat(sendMeagess,"/",1);
	  strncat(sendMeagess,rPacket->CARD_ID,18);
    strncat(sendMeagess,"/",1);
...
void SendOpenLockPacket(void)
{
	...
	refreshOpenLockPacket(&openLockPacket);
	encodeOpenLockPacket(sendBuffer,&openLockPacket);

	if(GPRS_TcpSendMessage(sendBuffer))
	{
			//printf("GPRS_TcpSendMessage ---------> OK\r\n");
			//printf("SendOpenLockPacket: %s \r\n",sendBuffer);
		
		  LogWrite(Send,"ReportOpenLock",sendBuffer);
	}
	else
	{
			//printf("GPRS_TcpSendMessage ---------> ERROR");
			//Sim80x.GPRS.Connection[0] = GPRSConnection_ConnectFail;
		  LogWrite(Send,"ReportOpenLockError",sendBuffer);
	}
